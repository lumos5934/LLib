...
namespace LLib
{
    public class ResourceManager : SingletonGlobal<ResourceManager>, IResourceManager, IPreInitializable
    {
        [SerializeField]
        ...
        public UniTask<bool> InitAsync(PreInitContext ctx)
        {
            ...
            foreach (var group in _groups)
            {
                ...
                if (!_allGroups.ContainsKey(group.Label)) _allGroups[group.Label] = new List<ResourceGroup>();
                ...
                foreach (var resource in resources)
                    if (!_allResources.TryAdd(resource.name, resource))
                        Debug.LogWarning(
                            $"fail add {group.Label} resources: {resource.name} (label : {group.Label}, path: {group.FolderPath})");
            }
        ...
        {
            if (_allResources.TryGetValue(assetName, out var resource)) return GetTypeResource<T>(resource);
